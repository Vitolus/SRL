    ...
    with open(path, 'r') as f:
        ...
        for line_no, line in enumerate(f):
            ...
            if not line:
                ...
                try:
                    assert len(roles) == len(frames), f'Line {line_no}: Lengths of roles and frames do not match.'
                except:
                    print(
                        f'Line {line_no}: Lengths of roles ({len(roles)}) and frames ({len(frames)}) do not match.\nArguments: {[r for r in roles[0] if r != "_"]}')
                roles_dict = {}
            ...
            if line.startswith('#'):
                ...
                assert key in {'united_srl_id', 'document_id', 'sentence_id', 'domain',
                               'text'}, f'Error in line {line_no}: {line}\nKey {key} is not valid.'
                ...
                assert not token_ids or token_id == token_ids[
                    -1] + 1, f'Error in line {line_no}: {line}\nToken ID {token_id} is not consecutive.'
    ...
    for united_srl_id, gold_sentence in gold_data.items():
        ...
        for token_id, gold_roles in gold_sentence['roles'].items():
            if token_id in pred_sentence['roles']:
                ...
                logging.warning(f'UniteD-SRL ID {united_srl_id}, token ID {token_id} is not in the prediction data.')
                pred_roles = []
            for gold_span, gold_role in enumerate(gold_roles):
                if gold_role == 'B-V':
                    continue
                if gold_role not in role_f1:
                    role_f1[gold_role] = {'tp': 0, 'fp': 0, 'fn': 0}

                if gold_role in pred_roles and gold_span < len(pred_roles):
                    pred_role = pred_roles[gold_span]
                    if pred_role != 'B-V' and pred_role not in role_f1:
            ...
            for pred_span, pred_role in enumerate(pred_roles):
                if pred_role == 'B-V':
                    continue
                if pred_role not in gold_roles:
